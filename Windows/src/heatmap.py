...
class Heatmap():
    ...
    def getAxis(self):
        a_number = self.min_x
        x_axis = []
        while a_number <= (self.max_x - self.min_x):
            x_axis.append(a_number)
            a_number += self.inc_step

        y_axis = []
        a_number = self.min_y
        while a_number <= (self.max_y - self.min_y):
            y_axis.append(a_number)
            a_number += self.inc_step
    ...
    def getAcopladeHist(self):
        int_x = 0
        int_y = 0
        arry = [[0 for _ in range(len(self.axis_x/self.inc_step))] for _ in range(len(self.axis_y/self.inc_step))]
        for value in self.axis_x/self.inc_step:
            for o_value in self.axis_y/self.inc_step:
                arry[int_x][int_y] = value + o_value
                int_y += 1
            int_x += 1
